...
if(infile=='../non_TS/summary-properties_corrected_webs.tsv'){
  save.image('nonTS_models_corrected.Rdata') } else {
    save.image('TS_models_corrected.Rdata')
  }
...
write.table(summary(obs_LS)$coefficients,file=paste(outdir,'coefficients/LS_obs_corrected.tsv',sep=''),col.names=TRUE,row.names=TRUE,sep='\t')
write.table(summary(obs_Gen)$coefficients,file=paste(outdir,'coefficients/Gen_obs_corrected.tsv',sep=''),col.names=TRUE,row.names=TRUE,sep='\t')
write.table(summary(obs_Vul)$coefficients,file=paste(outdir,'coefficients/Vul_obs_corrected.tsv',sep=''),col.names=TRUE,row.names=TRUE,sep='\t')
...
if(infile=='../non_TS/summary-properties_corrected_webs.tsv'){
  ...
  write.table(Gen_marg,file=paste(outdir,'marginals/Gen_S_marginal_corrected.tsv',sep=''),sep='\t',col.names=TRUE)
  Vul_marg=S_CIs("Vul_min")
  write.table(Vul_marg,file=paste(outdir,'marginals/Vul_S_marginal_corrected.tsv',sep=''),sep='\t',col.names=TRUE)
}
