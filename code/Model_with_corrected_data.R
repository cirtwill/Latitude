...
# for(infile in c('../mod_data/summary-properties_extended_connected.tsv','../non_TS/summary-properties_extended_connected.tsv')){
  ...
  if(infile=='../mod_data/summary-properties_extended_connected.tsv'){
    Gen_min=with(data,lm(log10(Gen)~log10(Species)
      +log10(Species):(Lake+Marine+Stream+Terr)
      +log10(Species):Latitude
      +log10(Species):Latitude:Lake
      +log10(Species):Latitude:Stream
      ,na.action=na.fail)) } else {
    Gen_min=with(data,lm(log10(Gen)~log10(Species)
      +log10(Species):(Lake+Marine+Terr+Stream)
      +log10(Species):Latitude
  ...
  if(infile=='../mod_data/summary-properties_extended_connected.tsv'){
    newdata=matrix(nrow=200*3*4,ncol=6)
    k=1
    for(j in c("Lake","Marine","Terr","Stream")){
    ...
    colnames(newdata)=c("Species","Latitude","Lake","Marine","Terr","Stream") } else {
    newdata=matrix(nrow=200*3*4,ncol=6)
    k=1
    for(j in c("Lake","Marine","Terr","Stream")){
      for(latitude in c(0,30,60)){
    ...
    colnames(newdata)=c("Species","Latitude","Lake","Marine","Terr","Stream")    }
