...
\section*{Introduction} 
  ...
  Food webs --networks of feeding links between species-- have been used for several decades to summarise the structure of 
...
and vulnerability were re-scaled to remove the effects of latitude and, where applicable, ecosystem
type by dividing the observed value of link density, generality, or vulnerability by species richness
or the proportion of species at a given trophic level raised to an exponent including the effects of
latitude, ecosystem type, and their interaction. Note that estuarine food webs were used as the intercept
ecosystem type in all models, but that at most two ecosystem types had interactions between ecosystem type and
latitude retained in the best-fit model (see \emph{results} for specifics). For each relationship, 
we show these rescaled values (white circles), as well as the overall scaling relationship using estuarine
ecosystems as a baseline (N=163 food webs). For a figure with the uncorrected values,
...
towards the poles. See \emph{Fig. S2, Appendix 5} for  all scaling
...
scaling of generalitywith the proportion of top predators where there was no effect of latitude on the 
size of the exponent in any ecosystem type. See \emph{Figs. S3-S5, Appendix 5} for all scaling relationships.}
\label{BIT}
\end{figure}
