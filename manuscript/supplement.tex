...
\subsection*{Appendix S1: details of food web sources and selection}
 ...
 See Table~\ref{S1} for a list of rejected webs and reasons for their
  ...
  \begin{landscape}
    ...
    it was ultimately included in the study. Collection 1 is~\cite{Riede2011}, collection 2 is~\cite{Dunne2013}, and collection 3 is~\cite{GlobalWeb}.}
    \label{S1}
    ...
    {\footnotesize
      \begin{tabular}{p{2.8cm}p{1.3cm}p{5.5cm}p{2.2cm}p{2.5cm}lp{3.5cm}}
        ...
        \hline
        Alamitos & 1 & \cite{Harrison1995}    & stream & 37,12,25 N & Yes   &       \\
        Alford & 1 & \cite{Havens1992}    & lake  & 44,15,41.59 N & Yes   &       \\
        Beaver & 1 & \cite{Havens1992}    & lake  & 44,18,58.13 N & Yes   &       \\
        Bere  & 1 & \cite{Woodward2008}  & stream & 50,45,7 N & Yes   &       \\
        Bridge & 1 & \cite{Havens1992}    & lake  & 44,09,42.66 N & Yes   &       \\
        Broad & 1 & \cite{Townsend1998}  & stream & 45.896938 S & Yes   &       \\
        Broadstone & 1 & \cite{Woodward2005} & stream  & 51,01,05 N & Yes   &       \\
        Calero & 1 & \cite{Harrison1995}    & stream & 37,11,09 - 20 N & Yes   &       \\
        Canton & 1 & \cite{Townsend1998}  & stream & 45.752 S & No    & Duplicated in GlobalWeb (web 235) \\
        Carpinteria2006 & 1 & \cite{Lafferty2006}    & estuary & 34.41 - 34.40 N & Yes   &       \\
        Chub  & 1 & \cite{Havens1992}    & lake  & 43,31,39.36 N & Yes   &       \\
        Coachella & 1 & \cite{Polis1991a} & terrestrial & 33,54 N & Yes   &       \\
...
\subsection*{Appendix S3: testing for effects of sampling effort}
  ...
  \subsubsection*{Results}
    ...
    in the methods used to compile food webs, these differences do not explain the trends we report in this study.
...
\subsection*{Appendix S4: Scaling relationships by trophic level}
  ...
  \subsubsection*{Methods}
    ...
    We were also interested in the ways that scaling
    relationships with species richness might
    be affected by changes to the distribution of species 
    among trophic levels. [[more justification of why]]
    To that end, we repeated our 
    analyses replacing species richness with the
    proportion of basal resources, proportion of 
    intermediate consumers, or proportion of top 
    predators. All model fitting procedures were identical 
    to those described for species richness. The model 
    simplicication procedure was similar to
    the procedure described for species richness, except 
    that in some cases there were multiple models with 
    $\delta$AIC\textless2 that shared the minimal degrees 
    of freedom. In these cases, we defined the 
    best-fitting model as the model with the lowest AIC in 
    the set of models with $\delta$AIC\textless2 and the 
    minimal degrees of freedom.


  \subsubsection*{Results}

    As with species richness, specialisation scaled with
    the numbers and proportions of species at each trophic
    level following a lognormal distribution.
    ...
    Fig.~\ref{TL_rescaled}). In each case, however, there were substantial
    ...
    $p$\textless0.001, respectively; Fig.~\ref{B}). For the scaling of link
    ...
    $\beta_0$=0.771, $p$\textless0.001 respectively; Fig.~\ref{TL_rescaled}). 
    ...
    and $\beta_{Latitude:Stream}$=-0.035, $p$=0.001; Fig~\ref{I}).
    ...
    Fig~\ref{TL_rescaled}). For estuarine, lake, and marine food webs the
    ...
    for vulnerability; Fig.~\ref{T}. The best-fit model for generality did not include any terms
...
\begin{figure}[!h]
\centerline{\includegraphics*[height=.5\textheight]{Figures/by_TL/scaling_with_S/proportions/TL_fitlines_nonts.eps}}
\caption{Scaling relationships for re-scaled link density, generality, and vulnerability 
relative to the proportions of basal resources, intermediate consumers, and top predators in a food web. 
Link density, generality,
...
see Fig.~\ref{TL_observed}.}
\label{TL_rescaled}
\end{figure}


\begin{figure}[!h]
\centerline{\includegraphics*[height=.75\textheight]{Figures/by_TL/scaling_with_S/proportions/TL_fitlines_nonts_observed.eps}}
\caption{Scaling relationships for link density, generality, 
and vulnerability relative to the proportions of basal resources, intermediate consumers, and top predators in a food web. 
For each relationship, we show observed values (white circles) and 
a simplified form of the scaling relationship described in equation 2 (\emph{Main Text}), neglecting 
any effects of habitat type or latitude (black line, N=196 food webs). See Fig.~\ref{TL_rescaled} to compare with 
observed values correcting for the effects of habitat type and latitude. }
\label{TL_observed}
\end{figure}
...
basal resources (black line) with its 95\% confidence interval (in grey), based on N=196 
...
N=196 empirical food webs. Latitude is given in degrees
from the equator regardless of direction.}
\label{I}
\end{figure}
...
\caption{Changes to the scaling of link density, generality, and vulnerability across ecosystem
types and over latitude. For each property we show the scaling exponent for the proportion of
top predators (black line) with its 95\% confidence interval (in grey), based on N=196 
empirical food webs. Latitude is given in degrees
from the equator regardless of direction.}
\label{T}
\end{figure}
...
\bibliographystyle{prsb}%Compile with jae.bst style file
